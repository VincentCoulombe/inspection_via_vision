...
@click.option("--load_dir", type=str, default="", help="Directory to load the model")
...
def main(lr: float, batch_size: int, epochs: int, save_dir: str, data_dir: str, load_dir: str, weights_saving_name: str):
    
	model = LargeMobileNet()
	if load_dir != "":
		model.load_state_dict(torch.load(os.path.join(load_dir, weights_saving_name)))
